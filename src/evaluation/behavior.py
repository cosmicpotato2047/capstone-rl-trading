...
주요 기능:
  ...
  4. gap_series()    : 실제 gap 값 시계열 (buy_hi_gap, sell_market_gap 등)
...
def actions_to_gaps(actions: np.ndarray) -> pd.DataFrame:
    """
    action 배열 (N, 2)을 ATR 배수 계수값으로 변환한다.
    ...
    Returns:
        DataFrame with columns:
            ...
            buy_hi_gap    : 0.5 + aggressiveness * 1.5  ([0.5, 2.0] × ATR)
            buy_lo_gap    : 2.5 + aggressiveness * 7.5  ([2.5, 10.0] × ATR)
            sell_market_gap : 0.5 + profit_target * 1.5  ([0.5, 2.0] × ATR)
            sell_cost_gap   : 2.5 + profit_target * 7.5  ([2.5, 10.0] × ATR)
    """
    agg = actions[:, 0]
    pt  = actions[:, 1]
    ...
    df = pd.DataFrame({
        "aggressiveness":   agg,
        "profit_target":    pt,
        "buy_hi_gap":       0.5 + agg * 1.5,   # [0.5, 2.0] × ATR
        "buy_lo_gap":       2.5 + agg * 7.5,   # [2.5, 10.0] × ATR
        "sell_market_gap":  0.5 + pt  * 1.5,   # [0.5, 2.0] × ATR
        "sell_cost_gap":    2.5 + pt  * 7.5,   # [2.5, 10.0] × ATR
    })
    return df
...
def collect(
    ...
    Returns:
        DataFrame, 1행 = 1스텝:
            ...
            aggressiveness, profit_target,
            buy_hi_gap, buy_lo_gap, sell_market_gap, sell_cost_gap,
    ...
    result_df = pd.concat(
        [result_df, gaps[["buy_hi_gap", "buy_lo_gap", "sell_market_gap", "sell_cost_gap"]]],
        axis=1,
    )
...
def print_regime_summary(behavior_df: pd.DataFrame) -> None:
    ...
    cols = ["aggressiveness", "profit_target", "buy_hi_gap", "sell_market_gap"]
    ...
    print(f"{'레짐':<6} {'Aggressive':>11} {'ProfitTgt':>10} "
          f"{'BuyHiGap(x)':>12} {'SellMktGap(x)':>14}")
    print("-" * 58)
    for regime, row in summary.iterrows():
        print(
            ...
            f"{row['buy_hi_gap']:>12.4f} "
            f"{row['sell_market_gap']:>14.4f}"
        )
