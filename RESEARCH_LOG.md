...
## 2026-05-16 — §9 Conclusion + 본 논문 최종 검증 완료 (양 버전)
...
| 파일 | 변경 |
|---|---|
| `reports/paper/main_ko.tex` | §9 TBD placeholder → 본문 + Future work (~1 페이지) |
| `reports/paper/main.tex` | §9 TBD placeholder → 본문 + Future work (~1 페이지) |

### §9 구조 (양 버전)

- 4 paragraph 메인 메시지:
  1. Pareto frontier trade-off (§5 reference)
  2. 세 환경 세 winner reversal (§7.2 + §7.3 reference)
  3. Prospect Theory OOS robust (H5, §7.3 reference) — 행동경제학 인간 표준 vs RL best 매개변수 차이 부수 발견
  4. In-sample 우위 ↔ OOS robust trade-off (DSR 양면성, §7.3.5 mechanism reference)
- Future work 5개 bullets (§8.5 한계 1:1 mapping)
- GitHub URL 공개 명시
...
- 빌드 검증: `xelatex main_ko.tex` 3회 + bibtex → **43 페이지** (이전 42, +1)
- `pdflatex main.tex` 3회 + bibtex → **45 페이지** (이전 44, +1)
- §9 분량 ~3% 목표 정확 일치
...
### 본 논문 메인 contribution 요약

1. **시나리오 D (Pareto frontier)**: 4 reward variant 가 단일 winner 가 아닌 두 cluster 의 risk profile trade-off (within $|d|$<0.30, across >0.79, policy distance 2.22×)
2. **세 환경 세 winner reversal**: Val sym (1.871) → CPCV dsr (1.413) → Test pt (0.367/0.339, 양 source p<0.002)
3. **H5 (pt OOS robust) — 본 논문 진짜 contribution**: Kahneman-Tversky (1979) prospect theory 의 RL OOS 안전성 첫 정량 확인 (메커니즘: hold duration mean 1.4h max 6h vs DSR 4.58h max 169h)
4. **방법론적 권고**: RL 트레이딩 평가의 표준 protocol 로 세 환경 (Val + CPCV + Test) 동시 사용

### Decision

본 논문 본문 모두 작성 완료. 사용자 명시 승낙 후 main merge + push 진행 예정. 현재는 commit 만, main 머지 대기.

### 보류 아이디어 (post-submission)

- Figure 폰트/색상 polish (현재 raw matplotlib output)
- references.bib 의 3건 minor warning 수정 (henderson volume/number, liu2021bitcoin/wilder1978atr empty journal)
- §6.6 의 ``인과 사슬 box'' 시각화를 abstract figure 후보로 추가 검토 (현재는 phase15 menu_c)
- 페이지 길이 (한글 43p, 영문 45p) 가 학부 졸업 논문 표준 (30~50p) 의 상단. submission 시 trim 가능성.
