...
\midrule
H5\newline(사후) & Prospect Theory의 OOS 강건성 & \texttt{pt} 변형이 봉인 해제된 Test 환경에서 다른 모든 변형을 통계적으로 유의하게 초과하며, 그 메커니즘이 prospect theory의 손실 회피 ($\lambda > 1$)와 오목 효용 ($\alpha < 1$)이 학습시킨 짧은 holding 행동에 있음. \\
\bottomrule
\end{tabular}
