...
\title{
    ...
    \textbf{The Effect of Reward Function Design\\
    in RL-Based BTC Grid Trading:\\
    Pareto Frontier, Winner Reversal,\\
...
trains policies to exit within an average of 1.4\,h (max 6\,h), thereby avoiding the
...
design takes effect along a \emph{trade-off dimension in risk profile}
...
This project ran in two prior phases (Phase 1 and Phase 2) before the work
reported here. Phase 1 hypothesized that PPO RL would discover dynamic
volatility adaptation and outperform a fixed-grid baseline. Post-hoc
behavioral analysis, however, revealed that the trained policy converged to a
near-constant action $[1.0, 0.0]$ (\S\ref{sec:phase1_negative}), and that the
...
the Train partition), and Test (2024 onward, unsealed) environments concurrently.
...
simultaneous application of three environments is a methodological
...
\end{equation}
We use $n_{\text{splits}}=2$ and $n_{\text{buy\_orders}}=2$ (best of an Optuna
50-trial search during Phase 2 preliminary tuning). \texttt{threshold\_basis}
is fixed to
...
coefficient values come from the same Optuna search:

\begin{table}[!htbp]
\centering
\caption{Grid coefficients of the ATR baseline (best of 50 Optuna trials).}
...
hyperparameter decision} until exp035. After unsealing, we
...
\midrule
H5\newline(post-hoc) & Prospect-theoretic OOS robustness & \texttt{pt} statistically significantly exceeds every other variant on the unsealed Test, and the mechanism is the short holding behavior induced by prospect theory's loss aversion ($\lambda > 1$) and concave utility ($\alpha < 1$). \\
\bottomrule
\end{tabular}
...
a favorable-bias artifact. However, the qualitative observation central
to this section---that the \emph{policy converges to a constant}---is
...
becomes the principal source of RL's added value.
...
sits marginally behind \texttt{asym} on all metrics. There is no single winner;
...
In other words, the policy's trade frequency is determined primarily by the
...
an environment with only a 0.05\% fee. As a standard sim-to-real (sim2real)
stress test we now add slippage $\sigma = 0.02\%$ and ask whether Scenario D
(two clusters,
...
This subsection presents the paper's central result. We evaluate 100 RL models
...
to the exp035 stage. This subsection represents the \emph{first} Test
unsealing in this project; the Test statistics
...
\S\ref{sec:pareto} and the CPCV-1st of \S\ref{sec:cpcv} completes this paper's
central argument.
...
performance; in some cases it points in the opposite direction. This forces the
...
avoiding sell-side timing risk. MDD remains near $2.3\%$ and Sharpe remains
...
stretches to a median of 1\,h, p95 of 20\,h, and max of 169\,h (7 days). When BTC's price
...
\subsubsection{H5: Statement and Evidence}
\label{sec:oos_h5}
...
\item Add additional regime tests (2018 bear, 2019 sideways) via simulation or
historical tests.
