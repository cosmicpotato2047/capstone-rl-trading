...
variant. Boxes are IQR; the center line is the median; whiskers extend to
1.5$\,\times\,$IQR. \texttt{dsr} has the highest median and 5\% CVaR and the
tightest spread (std 0.378). \texttt{asym/pt} are wider (IQR
$0.5$\,--\,$1.5$) with lower medians. The ATR baseline Val Sharpe $1.378$
...
\subsubsection{Winner Reversal: The Evaluation Method Picks the Winner}
...
\caption{4 variants $\times$ 15 paths Sharpe heatmap. Each cell is a
single (variant, path) Sharpe; the right margin gives the row mean (matching
Table~\ref{tab:cpcv_per_variant}). The \texttt{dsr} row is consistently dark
(high Sharpe), while \texttt{asym/pt} show larger path-to-path variance.}
...
advantage and OOS failure. To our knowledge, this is the first quantitative
evidence that \citet{kahneman1979prospect}'s prospect theory confers
\emph{out-of-sample safety} on RL trading policies.
...
to the exp035 stage. This subsection (2026-05-16) represents the
\emph{first-ever} Test unsealing in this project; the Test statistics
...
B\&H is included as a secondary baseline added during Phase 16b.
...
in both sources and exceeds the ATR baseline ($-0.055$) by the largest margin.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{menu3_boxplot}
\caption{Test Sharpe distributions (variant $\times$ source). \texttt{pt}'s
box is highest in both sources; \texttt{dsr} drops below zero in the exp032b
source and has the widest spread in the exp034 source. The ATR baseline
$-0.055$ (dashed line) is the comparison reference.}
...
\subsubsection{Three Environments, Three Winners (Reversal)}
...
three winners'' as an honest quantitative result and discuss its scientific
...
\caption{Per-variant Val Sharpe vs Test Sharpe (both sources). Horizontal:
Val Sharpe; vertical: Test Sharpe. The dashed diagonal is the ``Val = Test''
reference. All points lie below the diagonal, but \texttt{pt}'s points drop
the least (starting from the lowest Val position and ending at the highest
Test position).}
...
\texttt{pt}   & $1.667 \to 0.367$ & $\mathbf{-1.30}$ \textbf{(smallest)} & $1.092 \to 0.339$ & $\mathbf{-0.75}$ \textbf{(smallest)} \\
\bottomrule
\end{tabular}
\label{tab:oos_gap}
\end{table}
...
paper's frame into an honest ``in-sample advantage $\leftrightarrow$
...
\texttt{dsr} fail OOS?''---is answered by trajectory analysis (Phase 16d).
...
flips sign} depending on the evaluation environment. The paper's frame
``in-sample advantage and OOS robustness as a trade-off along the same
dimension of reward formulation'' is thereby established by direct
...
\begin{table}[h]
\centering
\caption{Per-variant Val/Test behavioral deltas (40-model average, Phase 16d
Menu 2). \textit{Every variant has $\Delta \leq 5\%$ on every behavioral
statistic, i.e., the policies behave nearly identically.}}
\begin{tabular}{lcccc}
\toprule
...
Val $\to$ Test gap in Table~\ref{tab:oos_gap} is \textbf{not driven by policy
change but by the market's distribution shift}. The policies are ``robust''
in the sense that they output the same actions, but those actions produce
different outcomes in a different market. \S\ref{sec:discussion} quantifies
the distribution shift (KS $p < 10^{-10}$, variance $-27\%$).

\subsubsection{Honest Acknowledgment of the B\&H Baseline}
\label{sec:oos_bh_honesty}

For academic honesty we added a Buy-and-Hold (B\&H) baseline during Phase 16b.
...
Table~\ref{tab:bh_comparison} is the honest picture. \textbf{On absolute
...
We discuss this honestly in \S\ref{sec:discussion}.
...
across two sources at $p < 0.002$. To our knowledge, this is the first
quantitative demonstration that prospect theory confers OOS safety on RL
policies.
...
(ii) honest acknowledgment of the environment dependence of the prior
exp027\_rl evidence (\S\ref{sec:disc_env_dependence}), (iii) the limitations
...
the \emph{distribution shift of the market itself}. Phase 15b directly
...
\caption{KDE plots of the ATR ratio and hourly log return on Val
(2021--2023, blue) and Test (2024+, orange). Test is tighter and centered
at a positive mean log return. KS $p < 10^{-10}$ on both metrics.}
...
Phase 2, April 2026) was that a simplified prospect-style asymmetric reward
...
\emph{environment} exceeds the change due to \emph{reward variant}. We accept
this honestly:
\begin{itemize}[leftmargin=*]
...
variant differences are reported. Our analysis spans from the single-split
Cohen's $d$ pattern of \S\ref{sec:pareto_clusters}, through the CPCV
preservation of \S\ref{sec:cpcv_cluster}, to the three-environment reversal
of \S\ref{sec:oos_three_env_reversal}, quantifying how the evaluation
environment itself reshapes the distance metric between reward variants.
...
across two sources at $p<0.002$. To our knowledge, this is the first
quantitative OOS-safety result at the intersection of behavioral economics
and RL.
...
the honest multi-dimensional quantification, in place of the simpler
``variant X gives the best alpha'' single-metric conclusion.
...
(\S\ref{sec:oos_mechanism}). To our knowledge, this is the first
quantitative demonstration that behavioral-economics prospect theory confers
OOS safety on RL policies. A side finding is that
...
Five directions responding to the limitations of \S\ref{sec:disc_limitations}:
\begin{itemize}[leftmargin=*]
\item \textbf{Multi-asset verification}: re-test H5 on equity ETFs (S\&P 500
sectors), FX major pairs, and commodities (gold/oil). Distribution-shift
forms differ across assets; the question is whether
\texttt{pt}'s short-holding mechanism generalizes.
\item \textbf{Additional regimes}: extend Test to the 2018 bear, 2019
sideways, and potential 2025+ sideways periods. Broaden ``OOS robust'' from
``bull only'' to ``all regimes.''
\item \textbf{Domain Randomization (DR)}: train with randomized
slippage/fee/start times and ask whether \texttt{dsr}'s OOS failure
recovers, and whether the already-robust \texttt{pt} gains additional
margin.
\item \textbf{Meta-RL and continuous reward learning}: learn a meta-policy
that adapts $(\alpha, \lambda)$ online based on environment signals. H5
verifies that ``one-shot'' prospect parameters are OOS robust, but
time-varying parameters may be more robust in nonstationary environments.
\item \textbf{Other RL algorithms}: this paper used PPO alone. Replicate
the reward-variant comparison on SAC, TD3, IMPALA to separate algorithm
effects from reward effects.
\end{itemize}
