...
emit the same actions, but those actions produce different outcomes in the
...
thereby avoiding the sell-side timing risk of the unseen bull regime. This
...
\begin{tabular}{p{1.3cm}p{4cm}p{8cm}}
\toprule
...
\midrule
H5 (post-hoc) & Prospect Theory OOS robustness & \texttt{pt} statistically significantly exceeds every other variant on the unsealed Test, and the mechanism is the short holding behavior induced by prospect theory's loss aversion ($\lambda > 1$) and concave utility ($\alpha < 1$). \\
\bottomrule
\end{tabular}
...
to ATR baseline outperformance (no-slip / with-slip). The ATR baseline is re-measured
...
(ii) explicit statement of the environment dependence of the prior
