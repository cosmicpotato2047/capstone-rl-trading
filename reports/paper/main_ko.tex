...
\caption{4 variant $\times$ 3 vol regime의 거래/hold rate. 위 행: trade rate.
아래 행: hold rate. \texttt{dsr}의 hold rate (가운데 행 두 번째 패널)가
다른 변형의 약 2--6배로 두드러진다.}
...
rate이다. 거래 빈도는 \texttt{sym}과 거의 동일($0.073$ vs $0.073$)이면서, hold
rate는 $0.142$로 \texttt{sym}의 $0.074$ 대비 \textbf{1.9배}, \texttt{asym}의
$0.036$ 대비 \textbf{3.9배}, \texttt{pt}의 $0.031$ 대비 \textbf{4.6배}이다.
high\_vol에서는 격차가 더 벌어진다(\texttt{dsr} $0.120$ vs \texttt{pt} $0.020$,
즉 \textbf{6.0배}).

이 격차의 메커니즘은 \S\ref{sec:reward_variants}의 DSR 정식에서 직접 유도된다.
DSR step reward
\[
r_t^{\mathrm{dsr}} =
\frac{B_{t-1} \, \Delta A_t - \tfrac{1}{2} A_{t-1} \, \Delta B_t}
...
2D 분포. 각 셀의 점은 trajectory step, 빨간 십자는 평균. \texttt{sym/dsr}이
$a^{(0)} \approx 0.12$로 좁은 매수 그리드, \texttt{asym/pt}는 $a^{(0)} \approx
0.3$--$0.45$로 넓은 매수 그리드(즉 ``현재가에서 멀리 떨어진 호가''). \texttt{dsr}만
$a^{(1)} \approx 0.15$로 매도 그리드가 살짝 더 넓어 hold 시간을 확보.}
...
\texttt{sym/dsr}은 $\bar{a}^{(0)} \approx 0.12$로 매수 호가가 현재가에 매우 가깝다
...
counterfactual map. 4 variant 모두 divergence 큰 영역에서 다른 행동을 보이며,
변형 간 차이가 가장 큰 영역은 ($s_t^{(1)} < 0$, mid atr\_ratio) 셀.}
...
오버퍼포먼스(no-slippage / with-slippage). Phase 16a에서 ATR baseline을
같은 슬리피지로 재평가한 결과 Val Sharpe $0.835$ (MDD 15.27\%)로 감쇠.}
\begin{tabular}{lcccccc}
\toprule
...
슬리피지는 \emph{cluster 구조에 선택적이지 않은} 일률적 감쇠 효과로 작용한다.
...
\subsubsection{Cluster Preservation: 시나리오 D는 강건하다}
...
\caption{exp032b(연한색) vs exp033(진한색) 결과 비교. 각 variant마다 평균
$\pm$ std와 10 시드 점을 함께 표시. 4 variant 모두 슬리피지 추가 후 mean
Sharpe가 약 $0.20$ 감소하나 변형 간 순서 관계와 cluster 분리는 보존.}
...
\subsubsection{ATR Baseline 의 공정 비교 (Phase 16a)}
...
근소하게 초과하지만 차이가 작아 caveat 가 필요했다. Phase 16a (2026-05-16)에서
ATR baseline을 같은 슬리피지($0.02\%$)로 재평가한 결과 ATR Val Sharpe는
...
청소된다. 이 결과는 가설 \textbf{H2 weak (asym/dsr/pt $>$ ATR)}가 슬리피지
...
\textbf{cluster 구조가 multi-split에서 더 또렷이 분리된다}는 점이다.
...
\caption{CPCV 15 path의 variant별 Sharpe 분포 boxplot. 박스는 IQR, 가운데
선은 중앙값, 수염은 1.5 IQR. \texttt{dsr}이 중앙값과 5\% CVaR 모두 가장 높고
분포가 가장 좁다(std 0.378). \texttt{asym/pt}는 IQR이 0.5\,$\sim$\,1.5로 더 넓고
중앙값이 낮다. ATR baseline의 Val Sharpe $1.378$는 비교 참조선.}
...
\subsubsection{Winner Reversal: 평가 방법이 변형 순위를 결정한다}
...
Sharpe. 행 우측의 mean이 표 \ref{tab:cpcv_per_variant}의 SR mean과 일치.
\texttt{dsr} 행이 어두운 색(높은 Sharpe)이 일관, \texttt{asym/pt}는 path 간
편차가 크다.}
...
\textit{CPCV에서 within-cluster 차이가 크게 줄어 cluster 분리가 가장 또렷.}}
\begin{tabular}{lccc}
\toprule
...
\emph{분포 외 안전성}의 (본 논문이 알기로는) 첫 정량 확인이다.
...
결정 또는 분석 과정에도 노출하지 않았다. 본 절(2026-05-16)이 \emph{사상 첫
...
평가한다. B\&H는 Phase 16b에서 추가된 secondary baseline이다.
...
1위이며 두 source 모두 ATR baseline($-0.055$)을 가장 큰 폭으로 초과. \texttt{dsr}은
exp032b source에서 음수 Sharpe로 ATR보다 더 나쁨.}
...
\caption{Test Sharpe 분포 (variant $\times$ source). \texttt{pt}의 박스가 두
source 모두 가장 높은 위치, \texttt{dsr}은 exp032b source에서 음수 영역,
exp034 source에서도 가장 넓은 분포. ATR baseline ($-0.055$, 점선)은 비교
참조.}
...
\subsubsection{세 환경 세 Winner Reversal}
...
본 논문은 이 ``세 환경 세 winner reversal''을 정직한 정량 결과로 인정하며,
...
\caption{Variant별 Val Sharpe와 Test Sharpe 비교 (양 source). 가로축: Val
Sharpe, 세로축: Test Sharpe. 대각선은 ``Val = Test'' 기준선. 모든 점이
대각선 아래로 떨어지지만 \texttt{pt}의 점이 가장 적게 떨어진다(Val에서 가장
낮은 위치에서 출발하여 Test에서 가장 높은 위치에 도달).}
...
대한 메커니즘 답변을 trajectory 분석(Phase 16d)으로 제공한다. 핵심 척도는
...
\end{itemize}
즉 \textbf{같은 reward formulation의 같은 행동 효과가 평가 환경에 따라 반대
부호}로 작동한다. ``in-sample 우위와 OOS robust는 reward formulation의 동일
차원에서의 trade-off''라는 본 논문 frame이 직접 정량 입증된다.
...
Phase 16d Menu 2). \textit{모든 variant의 정책 행동이 Val/Test에서 $\Delta \leq
5\%$로 거의 동일.}}
\begin{tabular}{lcccc}
\toprule
...
shift}가 원인이다. 즉 정책은 ``robust''하게 같은 행동을 출력하지만 그 행동이
다른 시장에서 다른 결과를 낳는다. \S\ref{sec:discussion}에서 이 distribution
shift의 정량(KS $p < 10^{-10}$, variance $-27\%$)을 자세히 다룬다.

\subsubsection{B\&H Baseline 정직 인정}
\label{sec:oos_bh_honesty}

본 논문의 학술적 정직성을 위해 Phase 16b에서 Buy-and-Hold (B\&H) baseline을
추가로 평가했다.
...
표 \ref{tab:bh_comparison}이 정직한 그림이다. \textbf{단일 Sharpe 기준에서는
...
이 정직한 인정은 \S\ref{sec:discussion}의 본 논문 한계 절에 반영된다.
...
$p < 0.002$로 정량 확인한다. 이는 본 논문이 알기로는 prospect theory가 RL
정책에 부여하는 OOS 안전성의 첫 정량적 증거이다.
...
(ii) exp027\_rl 사전 증거의 환경 의존성 정직 인정(\S\ref{sec:disc_env_dependence}),
...
Phase 15b에서 두 partition의 시장 통계를 직접 비교했다.
...
\caption{Val(2021--2023, 파랑)과 Test(2024+, 주황) 시장의 ATR 비율 및 시간당
log return의 KDE 분포. Test 분포가 Val 대비 좁고 평균 수익률이 양수 방향으로
이동. KS p $< 10^{-10}$.}
...
본 논문의 동기에 있던 강한 사전 증거(2026년 4월 Phase 2 후기 관찰)는
...
변경에 의한 변화보다 큼}을 보인다. 본 논문은 이 사실을 정직하게 인정한다:
\begin{itemize}[leftmargin=*]
...
\emph{대체}하는 새 발견이며, 학술적 정직성을 유지하면서 contribution을 강화한다.
...
실제로 본 논문은 \S\ref{sec:pareto_clusters}의 single-split Cohen's $d$ 패턴에서
\S\ref{sec:cpcv_cluster}의 CPCV preservation, \S\ref{sec:oos_three_env_reversal}의
세 환경 reversal까지의 분석을 통해, 평가 환경이 reward 변형 사이의 거리 metric
자체를 변형하는 현상을 정량화했다.
...
응용 수준의 결론을 양 source 일관 $p<0.002$로 정량 확인했다. 이는 본 논문이
알기로는 행동경제학과 RL의 교차점에서 정량적으로 검증된 첫 OOS 안전성 결과이다.
...
결론보다 multi-dimensional trade-off의 정직한 정량화가 본 논문의 contribution이다.
...
본 논문이 알기로는 행동경제학의 prospect theory가 RL 정책에 부여하는 OOS 안전성의
첫 정량 결과이다. 또한 인간 표준값($\alpha=0.88, \lambda=2.25$)보다
...
본 논문의 5개 한계(\S\ref{sec:disc_limitations})에 대응하는 후속 연구 방향:
\begin{itemize}[leftmargin=*]
\item \textbf{Multi-asset 검증}: H5를 주식 ETF (S\&P 500 sector), FX 메이저
페어, 원자재(금/원유) 환경에서 검증. distribution shift의 형태가 자산별로 다른
가운데 \texttt{pt}의 짧은-holding 메커니즘이 일반화되는지 정량 측정.
\item \textbf{추가 regime 검증}: BTC 2018 bear, 2019 sideways, 2025+의 가능한
횡보 시기로 Test를 확장. ``OOS robust''의 정의를 bull-only에서 모든 regime으로
확장.
\item \textbf{Domain Randomization (DR) 통합}: 학습 시 slippage/fee/시작 시점을
randomize하는 DR 기법으로 \texttt{dsr}의 OOS 실패가 회복되는지 검증. naive
학습에서 OOS robust인 \texttt{pt}가 DR 학습에서 추가 우위를 보일지도 확인.
\item \textbf{Meta-RL과 continuous reward learning}: 환경 신호로 reward 매개변수
($\alpha, \lambda$)를 online 적응하는 meta-policy. 본 논문 H5는 ``one-shot''
prospect parameter가 OOS robust임을 확인했으나, 시변 환경에서 adaptive parameter가
더 robust할 가능성.
\item \textbf{다른 RL 알고리즘}: 본 논문은 PPO 단일 알고리즘. SAC, TD3, IMPALA
등 다른 actor-critic 방법에서 reward 변형 비교를 반복하여 algorithm 효과와
reward 효과의 분리.
\end{itemize}
